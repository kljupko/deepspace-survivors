...
if TYPE_CHECKING:
    ...
    from .menus import Info as InfoMenu
    from .menus import Pause as PauseMenu
    from .trays import  TopTray, BottomTray
...
class UnionDict(TypedDict):
    """A class representing a dictionary containing element union info."""
...
def build_upgrade_menu_elements(menu: UpgradeMenu):
    ...
    elements: list[ElementDict] = [
        ...
        _create_ElementDict(
                type='label',
                name='credits_amount',
                content=helper_funcs.shorten_number(menu.game.progress.data['credits']),
    ...
    for upgrade in menu.game.upgrades.values():
        ...
        # add upgrade name
        linked_elem_name = upgrade_dict['name']
        upgrade_dict = _create_ElementDict(
            type='textbox',
            name=upgrade.name.lower().replace(" ", "_") + "_name",
        ...
        # add upgrade textbox
        elem_content = upgrade.description
        elem_content += f"\nLevel: {upgrade.level}"
        ...
        upgrade_dict = _create_ElementDict(
            type='textbox',
            name=upgrade.name.lower().replace(" ", "_") + "_desc",
        ...
        elem_content = "Upgrade" if upgrade.is_available() else "   x   "
        if upgrade.max_level is not None and upgrade.level >= upgrade.max_level:
            elem_content = "Maxxed out"
        upgrade_dict = _create_ElementDict(
            type='label',
            name=upgrade.name.lower().replace(" ", "_") + "_btn",
            content=elem_content,
            linked_to=linked_elem_name,
            y_offset=1,
...
def build_rewards_menu_elements(menu: RewardsMenu):
    ...
    for reward in menu.game.rewards.values():
        ...
        reward_dict = _create_ElementDict(
            type='icon',
            name=reward.name.lower().replace(" ", "_") + "_icon",
            content=reward.image,
            linked_to=linked_elem_name,
            ignore_linked_x=True,
            y_offset=3
        )
        ...
        # add reward title
        linked_elem_name = reward_dict['name']
        reward_dict = _create_ElementDict(
            type='textbox',
            name=reward.name.lower().replace(" ", "_") + "_name",
            content=reward.name,
            linked_to=linked_elem_name,
            linked_anchor='topright'
        )
        ...
        # add reward instrucitions
        reward_dict = _create_ElementDict(
            type='textbox',
            name=reward.name.lower().replace(" ", "_") + "_instr",
            content=reward.instructions,
            linked_to=linked_elem_name
        )
        elements.append(reward_dict)
        ...
        reward_dict = _create_ElementDict(
            type='label',
            name=reward.name.lower().replace(" ", "_") + "_btn",
            content=elem_content,
            linked_to=linked_elem_name,
            y_offset=1,
            action=action
...
def build_settings_menu_elements(menu: SettingsMenu,
    ...
    elements: list[ElementDict] = [
        ...
        _create_ElementDict(
            type='label',
            name='show_fps_value',
            content=str(settings_data['show_fps']),
        ...
        _create_ElementDict(
            ...
            content=str(settings_data['music_volume']),
